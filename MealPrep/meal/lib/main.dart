...
class MealPrepApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Meal Prep',
      theme: ThemeData(
        primaryColor: const Color(0xFF5aa454),
        scaffoldBackgroundColor: const Color(0xFF6dbb63),
      ),
...
class _MainNavigationState extends State<MainNavigation> {
  ...
  void initState() {
    ...
    pages = [
      ...
      MealPlannerPage(
        mealPlan: _mealPlan,
        onWeekGroceriesChanged: (weekStart, items) {
          setState(() {
            _groceriesByWeek[weekStart] = items;
          });
        },
        getWeekGroceries: (weekStart) => _groceriesByWeek[weekStart] ?? <String>{},
      ),
      GroceryPage(
        groceriesByWeek: _groceriesByWeek,
        checkedGroceries: _checkedGroceries,
        onToggleItem: (weekStart, item) {
          setState(() {
        ...
        onDeleteWeek: (weekStart) {
          setState(() {
            // Remove all checked items for this week
            _checkedGroceries.removeWhere((key, value) => 
              key.startsWith('${weekStart.millisecondsSinceEpoch}_')
            );
            // Remove the grocery list for this week
            _groceriesByWeek.remove(weekStart);
            // Remove all meals for this week
            final weekEnd = weekStart.add(const Duration(days: 7));
            _mealPlan.removeWhere((date, meals) => 
...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const Scaffold(
      body: Center(child: Text("Home Page (built by teammate)")),
    );
  }
}
...
class RecipePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final sampleRecipes = <Meal>[
      Meal(
        "Chicken Stir Fry",
        ["Chicken", "Bell peppers", "Soy sauce", "Garlic"],
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          final meal = sampleRecipes[index];
          return Card(
            margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            child: ListTile(
